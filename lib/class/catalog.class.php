...
class Catalog extends database_object {
	...
	public static function get_catalog_ids() {
        ...
        /**
	 * get_catalog_ids, names, and type
	 * This returns an array of all catalog
	 */
        public static function get_all_catalogs() {
		$sql = "SELECT `id`, `name`, `catalog_type` FROM `catalog`";
		$db_results = Dba::read($sql);
	...
	public function get_art($catalog_id=0, $all='') {
            ...
            foreach ($albums as $album_id) {
                ...
                if ( time() > $ticker+1) {
                    ...
                    //flush();
                    update_text('Checking Art album ('.$search_count.' / '.count($albums).') : '.scrub_out($album->name));
                    $ticker = time();
                } //echos song count
	...
	public function add_to_catalog() {
		...
		foreach ($this->_playlists as $full_file) {
                    if ($this->import_m3u($full_file)) {
                        ...
                        if ($verbose) {
                            update_text(_('Added Playlist From') . " $file . . . .");
                            //flush();
                        }
		...
		//show_box_top();
		update_text(_('Catalog Update Finished')."... "._('Total Time')." [".date("i:s",$time_diff)."] ".
                    _('Total Songs')." [".$this->count."]");
		//show_box_bottom();
	...
	public function clean_catalog() {
		...
		/* Get all songs in this catalog */
                update_text('Searching songs from catalog : '.$this->name);
		...
		while ($results = Dba::fetch_assoc($db_results)) {
			...
			if (time() > $ticker+1) {
				...
				//flush();
                                update_text('Checking file '.$count.'/'.mysql_num_rows($db_results).' in catalog : '.$this->name);
				$ticker = time();
			} //echos song count
		...
		//show_box_top();
		//echo "<strong>";
		update_text('Catalog Clean Done. '.$dead_files.' file removed.');
		//echo "</strong><br />\n";
		//show_box_bottom();
		//flush();
	...
	public function verify_catalog() {
		...
		$cache = array();
		$videos = array();
                update_text('Searching videos from catalog : '.$this->name);
		...
		foreach ($cached_results as $results) {
			...
			if (is_readable($results['file'])) {
                            ...
                            if (time() > $ticker+1) {
                                ...
                                update_text('Verifying ('.$count.' / '.$number.') : '.scrub_out($file));
                                $ticker = time();
                            } //echoes song count
		...
		//show_box_top();
		update_text( _('Update Finished').' : '._('Checked').' '.intval($count).' / '. _('Updated').' '.intval($total_updated) );
		//show_box_bottom();
	...
	public static function optimize_tables() {
        ...
        update_text('Tables optimization...');
		$sql = "SHOW TABLES";
		$db_results = Dba::read($sql);
	 ...
	 */
	public static function check_album($album, $album_year = 0, $disk = 0, $mbid = '', $readonly = false) {
		...
		if (!$album) {
                    ...
                    //Check if albums with same name
                    $sql = "SELECT album.id FROM album WHERE album.`name` = '$album'";
                    $result = Dba::read($sql);
                    
                    //If 2 albums with same name
                    if(Dba::num_rows($result) > 1) {
                        
                        $albumIds = array();
                        
                        while($albumId = Dba::fetch_assoc($result)) {
                            $albumIds[] = $albumId['id'];
                        }
                        
                        //Check if years are same make a fusion
                        $sql = "SELECT year FROM song WHERE song.album IN (".implode(',', $albumIds).") GROUP BY year";
                        $result = Dba::read($sql);

                        if(Dba::num_rows($result) == 1) {

                            $goodId = $albumIds[0]; //Keep first albumId only 
                            unset($albumIds[0]);

                            //Put all songs in good album
                            $sql = "UPDATE song SET album = '".$goodId."' WHERE album IN (".implode(',', $albumIds).")";
                            Dba::write($sql);

                            //Set good year on good album
                            $year = Dba::fetch_row($result);
                            $sql = "UPDATE album SET year = '".$year[0]."' WHERE id = ".$goodId;
                            Dba::write($sql);

                            //Delete old albums
                            $sql = "DELETE FROM album WHERE id IN (".implode(',', $albumIds).")";
                            Dba::write($sql);
                        }
                    }
                }
