...
class Search extends playlist_object {
	...
	public function __construct($searchtype = 'song', $id = '') {
        ...
        /**
	 * Query search for artists
	 */
	public static function artistsLike($search) {

            if(strlen($search) < 2) {
                return;
            }
            
            $search = Dba::escape($search, $limit=5);
            $sql = "SELECT id, CONCAT_WS(' ', prefix, name) AS name 
                    FROM `artist` 
        ...
        /**
	 * Query search for albums
	 */
	public static function albumsLike($search, $limit=5) {
	 ...
	 */
	public static function songsLike($search, $limit=5) {
