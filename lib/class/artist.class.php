...
class Artist extends database_object {
	...
	public static function get_from_name($name) {
		...
		$sql = "SELECT `id` FROM `artist` WHERE `name` = '$name' LIMIT 1";
	 ...
	 */
        public static function get_all() {
            $sql = "SELECT artist.id, 
                           COUNT(DISTINCT(song.album)) AS nbAlbums,
                           CONCAT_WS(' ', artist.prefix, artist.name) AS name,
                           album AS lastAlbumId, 
                           image.id AS imId,
                           image.mime AS mime
                    FROM `song` 
                    LEFT JOIN artist ON song.artist = artist.id
                    LEFT JOIN image ON ( album = object_id AND object_type = 'album' AND image.size = 'original' )
                    GROUP BY artist
                    HAVING MAX(year) AND 
                           COUNT(artist) > 3
                    ORDER BY name";
            ...
            while($artist = Dba::fetch_assoc($result)) {
                ...
                $results[] = array(
                    ...
                    'name'        => $artist['name'],
                    'lastAlbumId' => $artist['lastAlbumId'],
                    'imId'        => $artist['imId'],
                    'ext'         => $ext,
                );
            }
