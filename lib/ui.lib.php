...
function access_denied($error = "Access Denied") {
	...
	update_text('<span class="error">Access Denied</span>');
	exit;
...
function update_text($value) {
	$txt .= "$('#ajax-status').html('".$value."');";
    
    $bufferSize = ini_get('output_buffering');
    
    if(is_numeric($bufferSize) && $bufferSize > strlen($txt)) {
        $txt .= str_repeat(' ', ($bufferSize - strlen($txt)));
    }
