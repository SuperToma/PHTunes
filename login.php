...
if (($_POST['username'] && $_POST['password']) ||
    (in_array('http', Config::get('auth_methods')) && ($_SERVER['REMOTE_USER'] || $_SERVER['HTTP_REMOTE_USER']))) {
    ...
    if (Config::get('demo_mode')) {
        ...
        if ($auth['success']) {
            ...
            die( _('Username or Password incorrect, please try again') );
        }
        ...
        if ($user->disabled) {
            ...
            die( _('User Disabled please contact Admin') );
        } // if user disabled
        elseif (Config::get('prevent_multiple_logins')) {
            ...
            if ($current_ip && ($current_ip != $session_ip)) {
                    ...
                    die( _('User Already Logged in') );
            } // if logged in multiple times
        } // if prevent multiple logins
        elseif (Config::get('auto_create') && $auth['success'] && !$user->username) {
            ...
            if (User::create($username, $name, $email, 
                ...
                $auth['success'] = false;
                die( _('Unable to create local account') );
            }
        } // End if auto_create
